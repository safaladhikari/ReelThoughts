...
public class AuthenticationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        System.out.println("[DEBUG] Session ID: " + (req.getSession(false) != null ? req.getSession(false).getId() : "No session"));
        System.out.println("[DEBUG] Session creation time: " + (req.getSession(false) != null ? req.getSession(false).getCreationTime() : "No session"));
        System.out.println("[DEBUG] Session last accessed time: " + (req.getSession(false) != null ? req.getSession(false).getLastAccessedTime() : "No session"));
        System.out.println("[DEBUG] Session max inactive interval: " + (req.getSession(false) != null ? req.getSession(false).getMaxInactiveInterval() : "No session"));
        ...
        if (uri.equals(contextPath + PROFILE)) {
            System.out.println("[DEBUG] Allowing access to profile page for logged-in user");
            System.out.println("[DEBUG] User attempting to access profile: " + userEmail);
            System.out.println("[DEBUG] User role for profile access: " + userRole);
            chain.doFilter(request, response);
            return;
        }
 ...
 * jakarta.servlet.http.HttpServletResponse;
 * 
 * import java.io.IOException; import com.reelthoughts.util.SessionUtil;
 * 
 * @WebFilter(asyncSupported = true, urlPatterns = "/*") public class
 * AuthenticationFilter implements Filter {
 * 
 * private static final String LOGIN = "/login"; private static final String
 * HOME = "/home"; private static final String REGISTER = "/register";
 * 
 * @Override public void init(FilterConfig filterConfig) throws ServletException
 * { // Initialization logic, if required }
 * 
 ...
 * System.out.println("[DEBUG] AuthenticationFilter: Processing URI: " + uri);
 * System.out.println("[DEBUG] Context Path: " + contextPath);
 * System.out.println("[DEBUG] Session ID: " + (req.getSession(false) != null ?
 * req.getSession(false).getId() : "No session"));
 * 
 * // Handle root URL - redirect to login if (uri.equals(contextPath) ||
 * uri.equals(contextPath + "/")) {
 * System.out.println("[DEBUG] Root URL detected, redirecting to login");
 * res.sendRedirect(contextPath + LOGIN); return; }
 * 
 * // Allow access to login, register, home, and static resources without
 * authentication if (uri.contains(LOGIN) || uri.contains(HOME) ||
 * uri.contains(REGISTER) || uri.contains("/css/") || uri.contains("/js/") ||
 * uri.contains("/images/")) {
 * System.out.println("[DEBUG] Allowing access to public resource: " + uri);
 * chain.doFilter(request, response); return; }
 * 
 * // Check if the user is logged in String userRole = (String)
 * SessionUtil.getAttribute(req, "userRole"); String userEmail = (String)
 * SessionUtil.getAttribute(req, "user");
 * 
 * System.out.println("[DEBUG] User Role: " + userRole);
 * System.out.println("[DEBUG] User Email: " + userEmail);
 * System.out.println("[DEBUG] Session exists: " + (req.getSession(false) !=
 * null));
 * 
 * // If not logged in, redirect to login if (userRole == null || userEmail ==
 * null) {
 * System.out.println("[DEBUG] User not logged in, redirecting to login");
 * res.sendRedirect(contextPath + LOGIN); return; }
 * 
 * // Handle admin access if (uri.contains("/admin/") &&
 * !"admin".equals(userRole)) {
 * System.out.println("[DEBUG] Non-admin user attempting to access admin area");
 * res.sendRedirect(contextPath + HOME); return; }
 * 
 * // Continue with the request
 * System.out.println("[DEBUG] Allowing access to protected resource: " + uri);
 * chain.doFilter(request, response); }
